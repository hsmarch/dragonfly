//
// Dragonfly InputManager.cpp - Harrison March
//
...
int df::InputManager::startUp()
{
	...
	p_window = GraphicsManager::getInstance().getWindow();
...
void df::InputManager::getInput()
{
	...
	while (p_window->pollEvent(event))
	{
		...
		{
			EventKeyboard e;
			e.setKeyboardAction(KEY_PRESSED);
		...
		{
			EventKeyboard e;
			e.setKeyboardAction(KEY_RELEASED);
			e.setKey(sfmlKeyToDfKey(event.key.code));
			onEvent(&e);
		}
	...
	{
		EventMouse e;
		e.setMouseAction(PRESSED);
		e.setMouseButton(LEFT);
		onEvent(&e);
	}
