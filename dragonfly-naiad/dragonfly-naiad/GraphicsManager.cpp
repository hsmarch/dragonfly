...
int df::GraphicsManager::startUp() {
  ...
  if(p_window == nullptr) {
    df::LogManager::getInstance().writeLog("GraphicsManager: Failed to make SFML window\n");
    return -1;
  }
  ...
  if(!font->loadFromFile(font_file)) {
    df::LogManager::getInstance().writeLog("GraphicsManager: Failed to load font \"%s\"\n", font_file.c_str());
    return -1;
  }

  Manager::startUp();
  df::LogManager::getInstance().writeLog("GraphicsManager: Graphics manager succesfully started\n");
  return 0;
}
...
int df::GraphicsManager::drawString(Vector pos, std::string str, Justification just, Color color) {
  ...
  if(success == 0) {
    ...
    df::LogManager::getInstance().writeLog("GraphicsManager: Failed to draw one or more characters in string\n");
    return -1;
  }
}
